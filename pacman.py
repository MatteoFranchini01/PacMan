...
class PacMan(Actor):
    ...
    def collide(self, other):
        ...
        elif isinstance(other, GhostAlt):
            if self._lives != 0:
                self._lives -= 1
                self._x, self._y = self._x_in, self._y_in
                self._dx, self._dy = 0, 0
...
class PacManGame:
    def __init__(self, level: str, alternative: str):
        ...
        if self._alternative == "1":
            ...
            self._hero = PacMan(self._arena, (8, 8), 3)
            self._ghost_alt = GhostAlt(self._arena, (88, 88))
...
class PacManGui:
    ...
    def tick(self):
        if self._welcome:
            ...
            if self._alternative == "1":
                self._level = g2d.prompt(
                    "Inserire il livello di difficoltà a cui si intende giocare.\nInserire E, M, H per scegliere il livello")
                if self._level == "e":
                    ...
                    g2d.alert(
                        "Hai selezionato il livello più difficile, hai a disposizione 2 vite e compariranno 4 fantasmi")
                else:
                ...
                g2d.alert(
                    "Il giocatore (PacMan) dovrà mangiare tutti i biscotti senza essere colpito dal giocatore (Fantasma)")
            else:
                g2d.alert("Valore Errato")
                self.tick()
            self._welcome = False
